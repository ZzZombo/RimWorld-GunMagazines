...
namespace ZzZomboRW
{
    public class PatchOperationAddNode : PatchOperationPathed
    {
        ...
        protected override bool ApplyWorker(XmlDocument xml)
        {
            ...
            //Log.Message($"Value XML for {this.xpath}: {this.value.node.OuterXml}, count: {nodes.Count}", true);
            foreach (var obj in nodes)
            {
                if (obj is XmlNode xmlNode)
                {
                    if (name.NullOrEmpty())
                    {
                        ...
                        //Log.Message($"Result XML for {this.xpath}: {xmlNode.OuterXml}", true);
                        result = true;
                        continue;
                    }
    ...
    public class CompGunWithMagazines : ThingComp
    {
        ...
        internal static class HarmonyHelper
        {
            ...
            {
                Harmony harmony = new Harmony("ZzZomboRW.GunMagazines");
                harmony.PatchAll();
            }
